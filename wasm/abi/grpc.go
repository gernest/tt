...
func ProxyOpenGrpcStream(instance *host.Instance, grpcServiceData int32, grpcServiceSize int32,
...
func ProxySendGrpcCallMessage(instance *host.Instance, calloutID int32, data int32, size int32, endOfStream int32) int32 {
	...
	return ctx.SendGrpcCallMsg(calloutID, host.NewIoBufferBytes(msg), endOfStream).Int32()
}
...
func ProxyGrpcCall(instance *host.Instance, grpcServiceData int32, grpcServiceSize int32,
	...
	calloutID, res := ctx.GrpcCall(string(grpcService), string(serviceName), string(method),
		host.NewIoBufferBytes(msg), timeoutMilliseconds)
	if res != WasmResultOk {
		return res.Int32()
	}
