...
func ProxySendHttpResponse(instance *host.Instance, respCode int32, respCodeDetailPtr int32, respCodeDetailSize int32,
	...
	additionalHeaderMap := host.HeaderMap(DecodeMap(additionalHeaderMapData))

	ctx := getImportHandler(instance)

	return ctx.SendHttpResp(respCode,
		host.NewIoBufferBytes(respCodeDetail),
		host.NewIoBufferBytes(respBody),
		&additionalHeaderMap, grpcStatus).Int32()
}

func ProxyHttpCall(instance *host.Instance, uriPtr int32, uriSize int32,
	...
	headerMap := host.HeaderMap(DecodeMap(headerMapData))
	...
	trailerMap := host.HeaderMap(DecodeMap(trailerMapData))
