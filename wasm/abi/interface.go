...
type ImportsHandler interface {
	...
	// config
	GetVmConfig() *host.IoBuffer
	GetPluginConfig() *host.IoBuffer
	...
	// l4
	GetDownStreamData() *host.IoBuffer
	GetUpstreamData() *host.IoBuffer
	ResumeDownstream() WasmResult
	ResumeUpstream() WasmResult

	// http
	GetHttpRequestHeader() *host.HeaderMap
	GetHttpRequestBody() *host.IoBuffer
	GetHttpRequestTrailer() *host.HeaderMap

	GetHttpResponseHeader() *host.HeaderMap
	GetHttpResponseBody() *host.IoBuffer
	GetHttpResponseTrailer() *host.HeaderMap

	HttpCall(url string, headers *host.HeaderMap, body *host.IoBuffer, trailer *host.HeaderMap, timeoutMilliseconds int32) (int32, WasmResult)
	GetHttpCallResponseHeaders() *host.HeaderMap
	GetHttpCallResponseBody() *host.IoBuffer
	GetHttpCallResponseTrailer() *host.HeaderMap

	ResumeHttpRequest() WasmResult
	ResumeHttpResponse() WasmResult
	SendHttpResp(respCode int32, respCodeDetail *host.IoBuffer, respBody *host.IoBuffer, additionalHeaderMap *host.HeaderMap, grpcCode int32) WasmResult

	// grpc
	OpenGrpcStream(grpcService string, serviceName string, method string) (int32, WasmResult)
	SendGrpcCallMsg(token int32, data *host.IoBuffer, endOfStream int32) WasmResult
	CancelGrpcCall(token int32) WasmResult
	CloseGrpcCall(token int32) WasmResult

	GrpcCall(grpcService string, serviceName string, method string, data *host.IoBuffer, timeoutMilliseconds int32) (int32, WasmResult)
	GetGrpcReceiveInitialMetaData() *host.HeaderMap
	GetGrpcReceiveBuffer() *host.IoBuffer
	GetGrpcReceiveTrailerMetaData() *host.HeaderMap

	// foreign
	CallForeignFunction(funcName string, param string) (string, WasmResult)
	GetFuncCallData() *host.IoBuffer
	...
	// custom extension
	GetCustomBuffer(bufferType BufferType) *host.IoBuffer
	GetCustomHeader(mapType MapType) *host.HeaderMap
}
