...
type Instance struct {
	...
	funcCache sync.Map // string -> *wasmer.Function
...
func NewWasmerInstance(vm *VM, module *Module, options ...InstanceOptions) *Instance {
	...
	if err != nil || wasiEnv == nil {
		ins.importObject = wasmer.NewImportObject()
		return ins
	}
...
func (w *Instance) Start() error {
	ins, err := wasmer.NewInstance(w.module.module, w.importObject)
	if err != nil {
		return err
	}
...
func (w *Instance) RegisterFunc(namespace string, funcName string, f interface{}) error {
	...
	w.importObject.Register(namespace, map[string]wasmer.IntoExtern{
		funcName: fwasmer,
	})
...
func (w *Instance) HandleError(err error) {
	var trapError *wasmer.TrapError
